...
export async function getCostumeVotes(costumeId) {
	try {
		let numVotes = await dbRunPromise(
			'SELECT COUNT(*) FROM costumes_votes WHERE costume_id = $1',
			[costumeId]
		);
		return numVotes[0];
	} catch (e) {
...
export async function createCostumeContestant(costumeContestant) {
	try {
		let result = await dbRunPromise(
			'INSERT INTO costumes AS c (teamname, s3key) VALUES ($1, $2)',
			[costumeContestant.teamname, costumeContestant.s3key]
		);
		return result;
	} catch (e) {
		console.log(e);
...
export async function createCostumeVote(costumeId, userId) {
	try {
		let result = await dbRunPromise(
			'INSERT INTO costumes_votes VALUES ($1, $2) ON CONFLICT (team_id) DO UPDATE SET costume_id = Excluded.costume_id;',
			[costumeId, userId]
		);
		return result;
	} catch (e) {
...
export async function getCostumeVote(userId) {
	try {
		let id = await dbRunPromise(
			'SELECT costume_id FROM costumes_votes WHERE team_id = $1',
			[userId]
		);
		return id[0];
	} catch (e) {
...
export async function deleteCostumeContestant(costumeId) {
	try {
		let result = await dbRunPromise(
			'DELETE FROM costumes_votes WHERE costume_id = $1',
			[costumeId]
		);
		let result2 = await dbRunPromise('DELETE FROM costumes WHERE id = $1', [
			costumeId
		]);
		return 'success';
	} catch (e) {
...
export async function getVotePercentages() {
	try {
		let result = await dbRunPromise(
			'select costume_id, COUNT(*) FROM costumes_votes GROUP BY costume_id'
		);
		return result;
	} catch (e) {
		console.log(e);
