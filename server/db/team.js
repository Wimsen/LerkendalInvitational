		...
		await Promise.all(
			teams.map(async team => {
				...
				let result = await dbRunPromise(
					'INSERT INTO teams AS t (username, password_hash, teamname, member1, member2, group_number) VALUES ($1, $2, $3, $4, $5, $6)',
					[
						team.email,
						hash,
						team.teamname,
...
export async function createTeam(newUser) {
	...
	try {
		...
		let result = await dbRunPromise(
			'INSERT INTO users AS u (username, password_hash) VALUES ($1, $2)',
			[username, hash]
		);
		return result;
	} catch (e) {
	...
	try {
		await Promise.all(
			matches.map(async match => {
				let result = await dbRunPromise(
					'INSERT INTO matches AS m (team1_id, team2_id, table_number, start_time) VALUES ($1, $2, $3, $4)',
					[
						match.team1_id,
						match.team2_id,
						match.table_number,
...
export async function resetTournament() {
	try {
		let result = await dbRunPromise(
			'DELETE FROM costumes_votes; DELETE FROM costumes; DELETE FROM matches; DELETE FROM teams; DELETE FROM users; DELETE FROM chat'
		);
		return result;
	} catch (e) {
		console.log(e);
...
export async function getMatchesByTeamId(id) {
	try {
		let matches = await dbRunPromise(
...
export async function registerMatchResult(matchId, winnerId, loserId) {
	try {
		// TODO transaction
		await dbRunPromise('UPDATE matches SET winner_id = $1 WHERE id = $2;', [
			winnerId,
			matchId
		]);
		let team1_points = await dbRunPromise(
			'SELECT count(*) from matches where winner_id = $1',
			[winnerId]
		);
		let team2_points = await dbRunPromise(
			'SELECT count(*) from matches where winner_id = $1',
			[loserId]
		);

		await dbRunPromise('UPDATE teams SET points = $1 where id = $2', [
			team1_points[0].count,
			winnerId
		]);
		await dbRunPromise('UPDATE teams SET points = $1 where id = $2', [
			team2_points[0].count,
			loserId
		]);
...
export async function isEligibleForRegisterResult(teamId, matchId) {
	try {
		let result = await dbRunPromise(
			'SELECT COUNT(*) FROM matches WHERE id = $2 AND (team1_id = $1 or team2_id = $1)',
			[teamId, matchId]
		);
		console.log(result);
