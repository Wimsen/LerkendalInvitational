...
costumeRouter.get('/vote', isAuthenticated, async (req, res) => {
	...
	try {
		...
		if (result)
			res
				.status(200)
				.send(JSON.stringify({ costumeId: result.costume_id }));
...
costumeRouter.get('/numvotes/:id', isAuthenticated, async (req, res) => {
	...
	try {
		...
		if (result)
			res.status(200).send(JSON.stringify({ numVotes: result.count }));
	...
	async (req, res) => {
		try {
			...
			for (let row of result) {
				...
				votePercentages[row.costume_id].percentage = (
					Number(row.count) /
					totalVotes *
					100
				).toFixed(2);
			...
			res
				.status(500)
				.send(JSON.stringify({ error: 'Soemthing went wrong' }));
		}
	}
);
